...
kmod-zfs-podman: src/zfs-$(ZFS_VERSION).tar.gz src/kernel-$(KERNEL_VERSION).rpm src/kernel-core-$(KERNEL_VERSION).rpm src/kernel-devel-$(KERNEL_VERSION).rpm src/kernel-modules-$(KERNEL_VERSION).rpm src/kernel-modules-core-$(KERNEL_VERSION).rpm
...
kmod-zfs: src/zfs-$(ZFS_VERSION).tar.gz src/kernel-$(KERNEL_VERSION).rpm src/kernel-core-$(KERNEL_VERSION).rpm src/kernel-devel-$(KERNEL_VERSION).rpm src/kernel-modules-$(KERNEL_VERSION).rpm src/kernel-modules-core-$(KERNEL_VERSION).rpm
	dnf install -y -q \
		src/kernel-$(KERNEL_VERSION).rpm \
		src/kernel-core-$(KERNEL_VERSION).rpm \
		src/kernel-devel-$(KERNEL_VERSION).rpm \
